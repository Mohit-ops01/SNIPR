...
('admin', 'admin@SNIPR.com', '$2a$10$...', 'ADMIN');
```
...
```bash
# Daily backup script
pg_dump SNIPR_db > backup_$(date +%Y%m%d).sql

# Compressed backup
pg_dump SNIPR_db | gzip > backup_$(date +%Y%m%d).sql.gz
```

### Restore
```bash
psql SNIPR_db < backup_20240101.sql
```
